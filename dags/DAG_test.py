...
def filterData(**kwargs):
    ...
    ti = kwargs['ti']
    data=ti.xcom_pull(key='data', task_ids='test_read_data')
...
def transformData(**kwargs):
    ti=kwargs['ti']
    filteredData=ti.xcom_pull(key='filteredData', task_ids='test_filter_data')
...
def storeData(**kwargs):
    ...
    ti=kwargs['ti']
    storage = ti.xcom_pull(key='transformedData', task_ids='test_transform_data')
...
with DAG(
    ...
    task1 = PythonOperator(
        task_id="test_read_data",
        python_callable=reading,
        # provide_context=True
    )
    task2 = PythonOperator(
        task_id="test_filter_data",
